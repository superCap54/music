...
if(!get_user("id") || !get_team("id")){
    ...
    if ( !isset($module['login_required']) || (isset($module['login_required']) && $module['login_required']) ) {
        if( $controller != "Home" && $controller != get_option("frontend_template", "Stackblue") && $controller != "Auth" && $method != "cron"){
    ...
    if($page != "profile" && $page != "auth" && $method != "logout" && $page != "dashboard" && $page != "platform"){
            ...
            if(!empty($module)){
                if( isset($module['role']) && $module['role'] ){
                    ...
                    if(!permission($page, "", true) && !is_ajax()){
                        redirect_to( base_url("platform/no_permissions") );
                    }
                }
